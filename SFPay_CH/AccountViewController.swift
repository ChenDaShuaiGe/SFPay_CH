...
class AccountViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    func tableView(tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        imgV.image = CHTools.createImage(UIColor.cyanColor(), size: imgV.frame.size, isCircle: false)
        return imgV
    }
    
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        tableView .deselectRowAtIndexPath(indexPath, animated: true)
        let alertView = UIAlertView(title: nil, message: NSString(format: "资产%d", indexPath.row+1), delegate: nil, cancelButtonTitle: "确定")
        alertView.show()
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.text = NSString(format: "资产%d", indexPath.row+1)
        return cell!
    }
